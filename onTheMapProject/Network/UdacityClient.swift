...
class UdacityClient: NSObject {
    ...
    enum Endpoints {
        ...
        var stringValue: String {
            ...
            case .getStudentLocations:
                return Endpoints.base + "/StudentLocation?limits=100&order=-updatedAt"
...
//MARK: LOG IN
    ...
    class func login(email: String, password: String, completion: @escaping (Bool, Error?) -> Void) {
        ...
        RequestHelpers.taskForPOSTRequest(url: Endpoints.udacityLogin.url, apiType: "Udacity", responseType:
        LoginResponse.self, body: body, httpMethod: "POST") { (response, error) in
            if let response = response {
                Auth.sessionId = response.session.id
                Auth.key = response.account.key
                getLoggedInUserProfile(completion: { (succes, error) in
                    if succes {
    ...
    class func getLoggedInUserProfile(completion: @escaping (Bool, Error?) -> Void) {
        RequestHelpers.taskForGETRequest(url: Endpoints.getLoggedInUserProfile.url, apiType: "Udacity", responseType: UserProfile.self) { (response, error) in
            if let response = response { //UserPRofile
    ...
    class func logout(completion: @escaping () -> Void) {
        ...
        if let xrsfCookie = xsrfCookie {
            request.setValue(xrsfCookie.value, forHTTPHeaderField: "X-XSRF-TOKEN")
        }
        ...
        RequestHelpers.taskForPOSTRequest(url: Endpoints.addLocation.url, apiType: "Parse", responseType:
        PostLocationResponse.self, body: body, httpMethod: "POST") { (response, error) in
       ...
       //MARK: Update Location
        ...
        let body = "{\"uniqueKey\": \"\(information.uniqueKey ?? "")\", \"firstName\": \"\(information.firstName)\", \"lastName\": \"\(information.lastName)\",\"mapString\": \"\(information.mapString ?? "")\" \"mediaURL\": \"\(information.mediaURL ?? "")\", \"latitude\": \(information.latitude ?? 0.0), \"longitude\": \(information.longitude ?? 0.0)}"
        RequestHelpers.taskForPOSTRequest(url: Endpoints.updateLocation.url,
        apiType: "Parse", responseType: UpdatedLocationResponse.self, body:
        body, httpMethod: "PUT") { (response, error) in
